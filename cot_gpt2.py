...
class InstructGPT:
    ...
    def prepare_dataset(self):
        ...
        dataset = load_dataset("causal-lm/cot_flan")



# Since there is an extra "input" column:
        # Combine prompts
        def combine_fields(example):
            input_text = example['input'] if example['input'] else ""
            full_text = f"{example['instruction']} {input_text} {self.tokenizer.eos_token} {example['output']}"
            return {'text': full_text}
        
        dataset = dataset.map(combine_fields, remove_columns=dataset['train'].column_names)
        return dataset
...
# CUDA out of memory issue on colab so maybe I gotta try something to 
# make it more suitable for running there. Or get the uploaded HF model
# and train using something else?
